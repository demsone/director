...
const evidence = new URL('../verification/', import.meta.url);
...
page.on('request', request => {
  if (request.url().endsWith('/api/chat')) {
    ...
    requests.push({ imageAttached: body.session.image.dataUrl.startsWith('data:image/'), promptId: body.session.prompt.id, feedbackAttached: !!body.session.feedback.raw, priorTurns: body.session.chat.length });
  }
});
...
try {
  ...
  await page.screenshot({ path: resolve('verification/ready.png'), fullPage: true });
  ...
  assert.deepEqual(requests.map(r => r.priorTurns), [0, 2]);
  assert.ok(requests.every(r => r.imageAttached && r.feedbackAttached && r.promptId === 'photography-review'));
  steps.push('Two real image-aware follow-ups: visible colours/positions and prior conversation recalled; all context resent');
  await page.screenshot({ path: resolve('verification/feedback-and-chat.png'), fullPage: true });
  ...
  assert.equal(await page.locator('#feedback section').count(), 0);
  steps.push('New critique keeps image for repeat use; reload clears state; no browser storage');
  ...
  await page.screenshot({ path: resolve('verification/failure.png'), fullPage: true });
  process.exitCode = 1;
} finally {
  await writeFile(new URL('live-browser-report.json', evidence), JSON.stringify(result, null, 2));
  await browser.close();
}
