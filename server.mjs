...
async function readBody(req) {
  ...
  for await (const chunk of req) {
    length += chunk.length;
    if (length > 36 * 1024 * 1024) throw new AppError('The image and conversation exceed the request size limit.', 413);
    chunks.push(chunk);
  }
...
export function createApp({ client = createModelClient(), store = new SessionStore() } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/api/health') return json(200, { app: 'director-v2', schemaVersion: 1 });
      ...
      if (req.method === 'POST' && path === '/api/feedback') {
        const body = await readBody(req);
        const image = uploadImage(body.image), prompt = getPrompt(body.promptId);
        ...
        const session = { id: randomUUID(), createdAt: now, updatedAt: now, title: image.name, image, prompt, systemInstruction, model, modelInfo: reply.modelInfo || { provider: 'LM Studio', requestedModel: model, responseModel: reply.model || model }, feedback, chat: [], usage: reply.usage };
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  server.listen(port, '127.0.0.1', () => console.log(`Director V2 · Persistent sessions · http://127.0.0.1:${port}`));
